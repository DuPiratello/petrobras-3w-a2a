...
Data: 2026-03-06
Atualizado para coerencia de escopo em: 2026-03-22

## Objetivo
Registrar as alteracoes aplicadas para corrigir falhas de execucao entre `gradio`, `fastapi` e `ollama` no ambiente com `podman-compose`, mantendo foco tecnico no dominio 3W.

## Problemas Identificados
1. Callback do Gradio usava `asyncio.run(...)` dentro do proprio ambiente async (risco de erro de event loop).
2. Inconsistencia de endpoint do Ollama (`/api/generate` vs `/v1/completions`).
3. Falta de tratamento robusto para indisponibilidade inicial do Ollama (startup race).
4. Imports do backend frageis dependendo do diretorio de execucao.
...
- Inclusao de `try/except` para erros HTTP (`httpx.HTTPError`).
- Inclusao de timeout explicito no cliente HTTP (`timeout=30.0`).
- Melhoria da resposta de erro com status e corpo.
...
Alteracoes:
- Tentativas de reconexao (3 retries) com backoff progressivo.
- Timeout explicito (`timeout=60.0`).
- Tratamento de dois formatos de resposta:
  - `/v1/completions` -> `choices[0].text`
  - `/api/generate` -> `response`
- Mensagens de erro de conexao com URL de destino.
...
Alteracao:
- Padronizacao do `OLLAMA_URL` default para `http://host.containers.internal:11434/v1/completions`.

Impacto:
- Remove divergencia entre endpoint esperado e endpoint configurado no container.
...
- Inclusao de fallback de import com `try/except ModuleNotFoundError`:
  - tenta import relativo (`from agents...`, `from utils...`)
...
Durante testes, houve conflito de nomes de container ja existentes (`container name ... is already in use`).

Para garantir uso da imagem/codigo atual:
...
## Nota de Escopo
Referencias antigas a agente financeiro nao fazem parte da premissa atual do projeto (foco 3W) e devem ser tratadas como legado a remover do backend/orquestrador.
